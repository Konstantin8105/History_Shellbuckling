...
int     PRUSIZ =  512;         /* preset block/buffer size    */
int     NCPW   =    4;         /* no. characters per word     */
 
#define MAXFD    25            /* max no. of file descriptors */
 ...
 
{
     ...
     if (fstat((int)*fd, &stbuf) == -1)  /* error in status request ?    */
       ...
       bksz = PRUSIZ/NCPW;             /* set 'bksz' = file block size   */
...
/*  F2CPTR:  change a FORTRAN-77 Character Descriptor Word (pointer)
 *           into equivalent C-language pointer to a character string
 *
 *  this routine is called in C via:
 *
 *  char         *f77des;     (  pointer to f77 char structure )
 *  static char  *cptr;       (  pointer to character string   )
 *  static int    clng;       (  character string length       )
 *
 *            F2CPTR_(f77des, &cptr, &clng)
 *           -------------------------------
 *
 *  NOTICE:  one must pass the address of 'cptr' and 'clng'
 *           in order to have them set by F2CPTR.  However,
 *           only the value of 'f77des' need be passed
 *           to F2CPTR since it is only used, not set.
 *
 *  input arguements:
 *  -----------------
 *
 *    f77des  =  FORTRAN-77 character string structure word
 *    ======
 *
 *  output arguements:
 *  ------------------
 *
 *    cptr    =  C-language pointer to character string
 *    ====
 *
...
F77PTR    *f77des;            /* FORTRAN-77 character string structure */
 ...
 
{
     ...
     *clng       =  f77des->leng;               /* store length (in bytes) */
 
     *cptr       =  (char *)f77des->addr;       /* store/cast C-pointer    */
