...
int     PRUBLK = 4096;         /* preset block/buffer size
                                  in bytes (= 512 words)      */
int     NCPW   =    8;         /* no. characters per word     */
...
extern char  *malloc();
extern char  *strcpy();
/* Deleted re-declaration of printf().  scc complained. */
/*
extern int    open();   CRAY cc compiler doesn't like this to be redeclared.
*/
extern int    read();
extern int    write();
extern int    close();
...
/*  FTN2CP:  change a FORTRAN-77 Character Descriptor Word (pointer)
 *           into equivalent C-language pointer to a character string
 *
 *  this routine is called in C via:
 *
 *  char         *f77des;     (  pointer to f77 char string   )
 *  static char  *cptr;       (  pointer to character string  )
 *  static int    clng;       (  character string length      )
 *
 *            FTN2CP_(f77des, &cptr, &clng)
 *           -------------------------------
 *
 *  NOTICE:  one must pass the address of 'cptr' and 'clng'
 *           in order to have them set by FTN2CP.  However,
 *           only the value of 'f77des' need be passed
 *           to FTN2CP since it is only used, not set.
 *
 *  input arguements:
 *  -----------------
 *
 *    f77des  =  FORTRAN-77 Character Descriptor word of the form;
 *    ======         (*notice, offset & length given in BITS)
 *
 *                            26        6             32
 *               +--------+------------------+-------------------+
 *    Cray-2:    | offset |           length |      address      |
 *               +--------+------------------+-------------------+
 *                   6                 26             32
 *
 *               +------------------+--------+-------------------+
 *    Cray-XMP:  |           length | offset |      address      |
 *               +------------------+--------+-------------------+
 *                            34        6             24
 *
 *               +--------+------------------+-------------------+
 *  Cray-XMP/EA: | offset |           length |      address      |
 *               +--------+------------------+-------------------+
 *                   6                 26             32
 *
 *
 ...
 *  ------------------
 *
 *    cptr    =  C-language pointer to character string of the form;
 *    ====           (*notice, of (offset) is given in BYTES)
 *
 *               +----+----------------------+-------------------+
 *    Cray-2:    | of |                      |      address      |
 *               +----+----------------------+-------------------+
 ...
 *                 3           29                     32
 *
 *    clng    =  C-language char string length (less null terminator)
 *    ====
 *
 */
...
char      *f77des;            /* FORTRAN-77 descriptor word structure */
char       **cptr;            /* pointer to string character pointer  */
long int   **clng;            /* pointer to string  length   pointer  */
 
{
 
     long int    fwrd;        /* local FORTRAN-77 discriptor word */
     long int    cwrd;        /* local C character pointer word   */
     long int    clen;        /* local C char-string length word  */
 
     /* cast the FORTRAN-descriptor-word into a local long int word */
 
     fwrd     =  (long int)f77des;           /* cast into temporary */
 
     /* construct the local C character pointer word */
 
     cwrd     =  (fwrd & (077 << 58)) | (0xffffffff & fwrd);
 
     /* extract the C character string length value */

     clen     =  (fwrd >> 35) & 0x7fffff;    /* get length (in bytes) */

     /* store C character pointer word and string length */
 
     *clng       =  (long int*)clen;  /* store/cast length (in bytes) */
     *cptr       =  (char *)cwrd;     /* store/cast C-pointer word    */
