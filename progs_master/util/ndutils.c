...
/* Clear out legnd structure. */
  memset(&legnd, NULL, sizeof(struct old_legend_type));
  strcpy(legnd.heading, "LEGEND");

/* Clear out leg structure. */
  memset(&leg, NULL, sizeof(struct legend_type));

/* Initialize lineStyle to line. */
  memset(plot.lineStyle, NULL, MaxLeg*sizeof(*(plot.lineStyle)));
  memset(plot.marker, NULL, MaxLeg*sizeof(*(plot.marker)));
