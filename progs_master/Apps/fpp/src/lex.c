        ...
        if (st_is(ST_BOS)) {
          ...
          if (symp) {
            ...
            if (n < endp - curtok->token && n < MAXNAMELEN) {
              ...
              ungetstr(curtok->token+n, (endp - curtok->token) - n);
              ungetsym(SYM_DUMMY);
              endp = curtok->token + n;
              *endp = 0;
            }
          ...
          if (!strncmp(curtok->token,"to",2)) {
            ungetstr(curtok->token+2, (endp - curtok->token) - 2);
            ungetsym(SYM_DUMMY);
            endp = curtok->token+2;
            *endp = 0;
            curtok->val = TK_DUMMY;
          ...
          if (!strncmp(curtok->token,"function",8)) {
            ungetstr(curtok->token+8, (endp - curtok->token) - 8);
            ungetsym(SYM_DUMMY);
            endp = curtok->token+8;
            *endp = 0;
            curtok->val = TK_KEY;
            ...
            ungetstr(curtok->token+9, (endp - curtok->token) - 9);
            ungetsym(SYM_DUMMY);
            endp = curtok->token+9;
            *endp = 0;
            curtok->val = TK_KEY;
