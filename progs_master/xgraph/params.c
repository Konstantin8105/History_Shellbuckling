 ...
 */
{
	...
	if (XParseColor(param_disp, param_cmap, name, color)) {
	    ...
	    else if (stricmp(name, "royal_blue") == 0) {
		color->pixel = 0x004169e1;
		XQueryColor(param_disp, param_cmap, color);
	    }
	...
	if (font_size = index(name_copy, '-')) {
	   *font_size       = '\0';
		...
		for (i = 0; i < font_count; i++)
		    if (*font_info = XLoadQueryFont(param_disp, font_list[i])) break;
