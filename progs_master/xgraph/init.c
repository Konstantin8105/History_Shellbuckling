...
#define DEF_TITLE_TEXT		"X Graph Title"
#define DEF_XUNIT_TEXT		"X-Axis"
#define DEF_YUNIT_TEXT		"Y-Axis"
...
#define DEF_NO_X_GRIDS		"0"
#define DEF_NO_Y_GRIDS		"0"
...
#define DEF_FMT_X		"%.2f"
#define DEF_FMT_Y		"%.2f"
#define DEF_FMT_EX		"%.3f"
#define DEF_FMT_EY		"%.3f"
#define DEF_DEVICE		"ps"
#define DEF_OUTPUT_DEVICE	D_XWINDOWS
#define DEF_DISPOSITION		"To File"
#define DEF_FILEORDEV		""
#define DEF_DOCUMENT		"off"
#define DEF_OUTFMT		"Fit To Page"
#define DEF_FIT_TO_PAGE		"on"
...
#define DEF_MARGIN_FLAG         "off"
#define DEF_HD_MARGIN_FLAG      "off"

/* Default X- margins in (percent, 0->100) */
#define DEF_MARGIN_LEFT		"2"
#define DEF_MARGIN_RIGHT	"2"
#define DEF_MARGIN_TOP		"2"
#define DEF_MARGIN_BOTTOM	"0"

/* Default HP- margins in (inches) */
#define DEF_HP_MARGIN_LEFT	"0.0"
#define DEF_HP_MARGIN_RIGHT	"0.0"
#define DEF_HP_MARGIN_TOP	"0.0"
#define DEF_HP_MARGIN_BOTTOM	"0.0"

/* Default ID- margins in (inches) */
#define DEF_ID_MARGIN_LEFT	"0.0"
#define DEF_ID_MARGIN_RIGHT	"0.0"
#define DEF_ID_MARGIN_TOP	"0.0"
#define DEF_ID_MARGIN_BOTTOM	"0.0"

/* Default PS- margins in (inches) */
#define DEF_PS_MARGIN_LEFT	"0.50"
#define DEF_PS_MARGIN_RIGHT	"0.25"
#define DEF_PS_MARGIN_TOP	"0.50"
#define DEF_PS_MARGIN_BOTTOM	"0.25"

/* Default TG- margins in (inches) */
#define DEF_TG_MARGIN_LEFT	"0.0"
#define DEF_TG_MARGIN_RIGHT	"0.0"
#define DEF_TG_MARGIN_TOP	"0.0"
#define DEF_TG_MARGIN_BOTTOM	"0.0"
 ...
 */
{
    ...
    param_set("NoXGrids",     INT,  DEF_NO_X_GRIDS);
    param_set("NoYGrids",     INT,  DEF_NO_Y_GRIDS);
    param_set("Format X",     STR,  DEF_FMT_X);
    param_set("Format Y",     STR,  DEF_FMT_Y);
    param_set("Format eX",    STR,  DEF_FMT_EX);
    param_set("Format eY",    STR,  DEF_FMT_EY);

    param_set("Device",       STR,  DEF_DEVICE);
    param_set("Disposition",  STR,  DEF_DISPOSITION);
    param_set("FileOrDev",    STR,  DEF_FILEORDEV);
    sprintf(buf, "%d", o); /* get program name into buf[*]  */
    param_set("Output Device",INT,  buf);
    param_set("Document",     BOOL, DEF_DOCUMENT);
    param_set("OutputFmt",    STR,  DEF_OUTFMT);
    param_set("Fit To Page",  BOOL, DEF_FIT_TO_PAGE);
    param_set("Scale",        DBL,  DEF_SCALE);

    /* Set the default margins */
    param_set("Margins",      BOOL, DEF_MARGIN_FLAG);	 /* marginFlag (-marg) */
    param_set("HD_Margins",   BOOL, DEF_HD_MARGIN_FLAG); /* marginFlag (-Marg) */

    param_set("MarginLeft",   INT,  DEF_MARGIN_LEFT);
    param_set("MarginRight",  INT,  DEF_MARGIN_RIGHT);
    param_set("MarginTop",    INT,  DEF_MARGIN_TOP);
    param_set("MarginBottom", INT,  DEF_MARGIN_BOTTOM);

    param_set("HP_Margin_L",  DBL,  DEF_HP_MARGIN_LEFT);
    param_set("HP_Margin_R",  DBL,  DEF_HP_MARGIN_RIGHT);
    param_set("HP_Margin_T",  DBL,  DEF_HP_MARGIN_TOP);
    param_set("HP_Margin_B",  DBL,  DEF_HP_MARGIN_BOTTOM);

    param_set("ID_Margin_L",  DBL,  DEF_ID_MARGIN_LEFT);
    param_set("ID_Margin_R",  DBL,  DEF_ID_MARGIN_RIGHT);
    param_set("ID_Margin_T",  DBL,  DEF_ID_MARGIN_TOP);
    param_set("ID_Margin_B",  DBL,  DEF_ID_MARGIN_BOTTOM);

    param_set("PS_Margin_L",  DBL,  DEF_PS_MARGIN_LEFT);
    param_set("PS_Margin_R",  DBL,  DEF_PS_MARGIN_RIGHT);
    param_set("PS_Margin_T",  DBL,  DEF_PS_MARGIN_TOP);
    param_set("PS_Margin_B",  DBL,  DEF_PS_MARGIN_BOTTOM);

    param_set("TG_Margin_L",  DBL,  DEF_TG_MARGIN_LEFT);
    param_set("TG_Margin_R",  DBL,  DEF_TG_MARGIN_RIGHT);
    param_set("TG_Margin_T",  DBL,  DEF_TG_MARGIN_TOP);
    param_set("TG_Margin_B",  DBL,  DEF_TG_MARGIN_BOTTOM);


    if (debug) {
      printf("\n");
      printf("InitSets():  after initializing margin parameters\n");
      printf("\n");
      printf("MarginLeeft, MarginRight  = %13d %13d\n",PM_INT("MarginLeft"), PM_INT("MarginRight"));
      printf("MarginTop,   MarginBottom = %13d %13d\n",PM_INT("MarginTop"),  PM_INT("MarginBottom"));
      printf("\n");
    }
    ...
    xtb_fg = 0x00eeeeee;  // light gray
    xtb_bg = 0x00000000;  // black
    xtb_bd = 0x00000000;  // black


    xtb_fg = 0x004169e1;  // royal blue
    xtb_bg = 0x00ffff70;  // light yellow
    xtb_bd = 0x00000000;  // black
               ...
               char *val ) 
{
    ...
    FS("-Marg  Include margins for X-screen");
    FS("-marg  Include margins for HP/ID/PS/TG hard devices");
    ...
    FS("-mrgl  Margin_Left   (fraction of width)\n");
    FS("-mrgr  Margin_Right  (fraction of width)\n");
    FS("-mrgt  Margin_Top    (fraction of height)\n");
    FS("-mrgb  Margin_Bottom (fraction of height)\n");

    FS("-**ml  Margin_Left   (inches)\n");
    FS("-**mr  Margin_Right  (inches)\n");
    FS("-**mt  Margin_Top    (inches)\n");
    FS("-**mb  Margin_Bottom (inches)\n");
    FS("\n");
    FS("       where '**' = 'hp', 'id', 'ps', 'tg'\n");

    exit(1);
}

#define ARG1(opt, name) \
 ...
 */
{
	...
	if (argv[idx][0] == '-') {
		...
		ARG2("-lf", "LabelFont",  FONT,  "missing font name");
                ...
                /* Doesn't make much sense to PW why this must be switched,
                   but it must be. */
                ARG2("-digy", "Format X",  STR,  "Missing C-String");
                ARG2("-digx", "Format Y",  STR,  "Missing C-String");
                ARG2("-digey","Format eX", STR,  "Missing C-String");
                ARG2("-digex","Format eY", STR,  "Missing C-String");
                ARG1("-a",    "Animate");
                /* End PW */
		ARG1("-rv",   "ReverseVideo");
		ARG1("-tk",   "Ticks");
		ARG1("-tkax", "TickAxis");
		ARG1("-bb",   "BoundBox");

                /* FCW */
                ARG2("-mrgl", "MarginLeft",   INT, "missing left margin");
                ARG2("-mrgr", "MarginRight",  INT, "missing rightt margin");
                ARG2("-mrgt", "MarginTop",    INT, "missing top margin");
                ARG2("-mrgb", "MarginBottom", INT, "missing bottom margin");

                ARG2("-hpml", "HP_Margin_L",  DBL, "missing HP margin left");
                ARG2("-hpmr", "HP_Margin_R",  DBL, "missing HP margin right");
                ARG2("-hpmt", "HP_Margin_T",  DBL, "missing HP margin top");
                ARG2("-hpmb", "HP_Margin_B",  DBL, "missing HP margin bottom");

                ARG2("-idml", "ID_Margin_L",  DBL, "missing ID margin left");
                ARG2("-idmr", "ID_Margin_R",  DBL, "missing ID margin right");
                ARG2("-idmt", "ID_Margin_T",  DBL, "missing ID margin top");
                ARG2("-idmb", "ID_Margin_B",  DBL, "missing ID margin bottom");

                ARG2("-psml", "PS_Margin_L",  DBL, "missing PS margin left");
                ARG2("-psmr", "PS_Margin_R",  DBL, "missing PS margin right");
                ARG2("-psmt", "PS_Margin_T",  DBL, "missing PS margin top");
                ARG2("-psmb", "PS_Margin_B",  DBL, "missing PS margin bottom");

                ARG2("-tgml", "TG_Margin_L",  DBL, "missing TG margin left");
                ARG2("-tgmr", "TG_Margin_R",  DBL, "missing TG margin right");
                ARG2("-tgmt", "TG_Margin_T",  DBL, "missing TG margin top");
                ARG2("-tgmb", "TG_Margin_B",  DBL, "missing TG margin bottom");
		...
		ARG1("-Marg","Margins");
		ARG1("-M",   "StyleMarkers");
		ARG1("-marg","HD_Margins");
                ARG2("-ofmt","OutputFmt", STR, "Missing output format");
		ARG1("-p",   "PixelMarkers");
		ARG1("-P",   "LargePixels");
		ARG1("-lnx", "LogX");
		ARG1("-lny", "LogY");
		...
		ARG2("-xgl", "NoXGrids",  INT, "missing no. x-grids");
		ARG2("-ygl", "NoYGrids",  INT, "missing no. y-grids");
		...
		if (strcmp(argv[idx], "-device") == 0) {
			...
			dflag = D_POSTSCRIPT;
		    else if (strcmp(argv[idx], "postscript") == 0)
			dflag = D_POSTSCRIPT;
