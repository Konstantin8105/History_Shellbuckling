...
int     PRUBLC =  1024;         /* preset block/buffer size    */
     ...
     *ierr =  0;                         /* clear the user error code */
     ...
     if (acc == -1)                    /* file does not exist */
       ...
       if (*(op+1)=='o')               /* user option says 'old' */
         ...
         printf ("\n");
         blkerr(errno, ierr);         /* extract error number  */
	 ...
	 if (*(op+1) == 'o')           /* set 'size' = for 'old' files  */
	   {
         ...
         blkerr(NULL, ierr);          /* clear error number            */
         }
       }
...

{
     ...
     *ierr =  0;                       /* clear the user error code */
     ...
     offset =  PRUBLC * (*blk - 1);    /* (byte_wise) offset for 'blk' */
...

{
     ...
     *ierr =  0;                       /* clear the user error code */
     ...
     offset =  PRUBLC * (*blk - 1);    /* (byte_wise) offset of 'blk' */
...

{
     ...
     if (fstat((int)*fd, &stbuf) == -1)  /* error in status request ?    */
       ...
       bksz = PRUBLC/NCPW;             /* set 'bksz' = file block size   */
