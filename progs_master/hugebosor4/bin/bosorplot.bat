...
set PLOTMSG = (${HUGEBOSOR4}/bin/bplot_msg.bat $method $psfile)
set loptions = ""
...
switch ($infile)
  ...
  case "${case}..R,Z_RingLocation*":
    ...
    if ($PLOT[1] == "${BUSHNELL}/bin/xgraph.${MACHINE}") then
      ...
      echo "" >> $outfile
      $GAWK -f ${HUGEBOSOR4}/bin/ringloc.awk pass=1 $linfile >> $outfile
      $GAWK -f ${HUGEBOSOR4}/bin/ringloc.awk pass=2 $linfile >> $outfile
      $PLOT $outfile >! $psfile
      $PLOTMSG
    endif
    breaksw
  ...
  case "${case}..R,Z_MERIDIAN*":
    ...
    if ($PLOT[1] == "${BUSHNELL}/bin/xgraph.${MACHINE}") then
      ...
      echo "=gridlines(0, 0)" >> $outfile
      echo "" >> $outfile
      $GAWK -f ${HUGEBOSOR4}/bin/geom.awk pass=1 $linfile >> $outfile
      $GAWK -f ${HUGEBOSOR4}/bin/geom.awk pass=2 $linfile >> $outfile
      $PLOT $outfile >! $psfile
      $PLOTMSG
    endif
    breaksw
