...
register int i, j=0, limit;
  ...
  qsort(f, (size_t) j, sizeof(int), compare_integer);
...
*/
int compare_integer(const void *n, const void *m)
{ 
  if ( *(int *) n < *(int *) m ) return -1;
  else if ( *(int *) n > *(int *) m) return 1;
  return 0;
}
...
/*
trncate: takes a double value, trucates toward zero, and returns a double.
*/
double trncate(double x)
{
